...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("示例：python yueyue_nacos.py -u http://127.0.0.1")
    )
...
def check(url):
    full_url = f"{url}/nacos/v1/core/cluster/nodes?withInstances=false&pageNo=1&pageSize=10&keyword="
    headers={'User-Agent': 'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1)',
'Accept': '*/*',
'Connection': 'Keep-Alive'}
    try:
        response = requests.get(full_url, headers=headers,timeout=5, verify=False)
        if response.status_code == 200 and '"code":200,"message":null,"data":' in response.text:
